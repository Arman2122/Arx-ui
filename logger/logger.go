// Package logger provides logging functionality for the Arx-ui panel with
...
const (
	...
	logFileName      = "arxui.log"            // Log file name
...
func InitLogger(level logging.Level) {
	newLogger := logging.MustGetLogger("arx-ui")
	...
	if consoleBackend := initDefaultBackend(); consoleBackend != nil {
		...
		leveledBackend.SetLevel(level, "arx-ui")
	...
	if fileBackend := initFileBackend(); fileBackend != nil {
		...
		leveledBackend.SetLevel(logging.DEBUG, "arx-ui")
		backends = append(backends, leveledBackend)
	}
