...
update() {
    confirm "This function will update all arx-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/Arman2122/Arx-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/arx-ui https://raw.githubusercontent.com/Arman2122/Arx-ui/main/arx-ui.sh
    chmod +x ${xui_folder}/arx-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/Arman2122/Arx-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del arx-ui
        rm /etc/init.d/arx-ui -f
    else
        systemctl stop arx-ui
        systemctl disable arx-ui
        rm ${xui_service}/arx-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/Arman2122/Arx-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${xui_folder}/arx-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false >/dev/null 2>&1
    else
        ${xui_folder}/arx-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true >/dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi
    
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the Arx-UI panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${xui_folder}/arx-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    ${xui_folder}/arx-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${xui_folder}/arx-ui setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${xui_folder}/arx-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "arx-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "arx-ui and xray stopped successfully"
        else
...
restart() {
    if [[ $release == "alpine" ]]; then
        rc-service arx-ui restart
    else
        systemctl restart arx-ui
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "arx-ui and xray Restarted successfully"
    else
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service arx-ui status
    else
        systemctl status arx-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "arx-ui Set to boot automatically on startup successfully"
    else
        LOGE "arx-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "arx-ui Autostart Cancelled successfully"
    else
        LOGE "arx-ui Failed to cancel autostart"
    fi
...
show_log() {
    if [[ $release == "alpine" ]]; then
        ...
        0)
            ...
            ;;
        1)
            grep -F 'arx-ui[' /var/log/messages
            if [[ $# == 0 ]]; then
                before_show_menu
            fi
            ;;
        *)
        ...
        0)
            ...
            ;;
        1)
            journalctl -u arx-ui -e --no-pager -f -p debug
            if [[ $# == 0 ]]; then
                before_show_menu
            fi
            ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-arx-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-arx-ui.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-arx-ui.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-arx-ui.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/arx-ui -z /usr/bin/arx-ui https://github.com/Arman2122/Arx-ui/raw/main/arx-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service arx-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status arx-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'arx-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled arx-ui)
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${xui_folder}/arx-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    *)
    ...
    local reloadCmd="systemctl restart arx-ui 2>/dev/null || rc-service arx-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    *)
    ...
    reloadCmd="arx-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}arx-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; arx-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
        1)
            LOGI "Reloadcmd is: systemctl reload nginx ; arx-ui restart"
            reloadCmd="systemctl reload nginx ; arx-ui restart"
            ;;
        2)  
            LOGD "It's recommended to put arx-ui restart at the end, so it won't raise an error if other services fails"
            read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; arx-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${xui_folder}/arx-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${xui_folder}/arx-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${xui_folder}/arx-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${xui_folder}/arx-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${xui_folder}/arx-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            ${xui_folder}/arx-ui setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        ${xui_folder}/arx-ui setting -listenIP 0.0.0.0 >/dev/null 2>&1
...
│  ${blue}arx-ui update-all-geofiles${plain}   - Update all geo files             │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}Arx-UI Panel Management Script${plain}                │
