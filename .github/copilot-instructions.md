# Arx-UI Development Guide

## Project Overview
Arx-UI is a web-based control panel for managing Xray-core servers. It's a Go application using Gin web framework with embedded static assets and SQLite database. The panel manages VPN/proxy inbounds, monitors traffic, and provides Telegram bot integration.
...
### Database Management
- DB path: Configured via `config.GetDBPath()`, typically `/etc/arx-ui/arx-ui.db`
...
- Logs: `{log_folder}/arxipl.log`, `arxipl-banned.log`
...
Both `install.sh` and `arx-ui.sh` follow these patterns:
...
- Binary: `/usr/local/arx-ui/`
- Database: `/etc/arx-ui/arx-ui.db`
- Logs: `/var/log/arx-ui/`
- Service: `/etc/systemd/system/arx-ui.service.*`

## Testing & Debugging
- Set `XUI_DEBUG=true` for detailed logging
- Check Xray process: `arx-ui.sh` script provides menu for status/logs
- Database inspection: Direct SQLite access to arx-ui.db
- Traffic debugging: Check `arxipl.log` for IP limit tracking
